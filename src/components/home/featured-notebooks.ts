...
const learningPapers = [
  ...
  samplePaper({
    ...
    shortName: '实践案例 2026',
    keywords: ['案例', '任务', '迁移'],
    content: '案例资料适合按任务、输入、输出和复盘四类整理。灵笔会把这些片段留在同一个工作本里，后续可以继续生成资料脉络、语音摘要或课堂内容。',
  }),
];
...
const knowledgePapers = [
  samplePaper({
    ...
    content: '个人知识管理可以按主题、项目、引用和待办四类组织。工作本需要保留原始资料和整理后的结论，避免把来源和自己的判断混在一起。',
  }),
  samplePaper({
...
const researchPapers = [
  ...
  samplePaper({
    ...
    shortName: '相关工作 2026',
    keywords: ['相关工作', '对照', '证据'],
    content: '相关工作整理要区分已验证结论、假设、数据限制和可复用方法。工作本可以把每篇资料的贡献和局限放在同一个脉络里，方便写综述或开题报告。',
  }),
];
